...
### 从领域资料到可追溯知识问答的一站式知识图谱工作台

**先定义知识，再构建图谱，最后让每个答案都能回到实体、关系与原始信息。**
...
[快速开始](#-五分钟启动) · [技术架构](docs/TECHNICAL_ARCHITECTURE.md) · [部署指南](docs/DEPLOYMENT.md) · [视频教程](#-视频教程) · [参与贡献](CONTRIBUTING.md)
...
很多知识库从“上传文档”直接跳到“向模型提问”。它们能找出相似片段，却很难回答三个更重要的问题：

- 这批资料里的核心概念、属性和关系究竟是什么？
- 自动抽取出的知识是否符合领域专家的理解？
- 回答中的结论来自哪个实体、哪条关系或哪段原始资料？

UniGraph 把缺失的知识工程过程补了回来：先设计并人工确认知识架构，再抽取实体与关系，随后构建多层索引，最终生成带有结构化引用的回答。

> UniGraph 不只是一个文档聊天界面，而是一条可以检查、编辑和持续演进的知识生产流水线。
...
| **使用知识** | 构建社区报告与向量索引，组合实体、关系、来源和全局概览 | 可追溯、可继续追问的知识回答 |
...
UniGraph 会将局部实体与关系、原始信息源和全局社区概览组织成统一上下文。回答中的引用可以继续展开，帮助用户判断结论来自哪里，而不是只接受一个无法验证的模型答案。

<details>
<summary><strong>查看检索与回答技术路线</strong></summary>

<br>

![UniGraph 检索与回答流程](docs/assets/architecture/retrieval-overview.png)

更完整的设计、构建、索引与检索流程见 [技术架构文档](docs/TECHNICAL_ARCHITECTURE.md)。
...
准备 Docker 24+ 与 Docker Compose v2：
...
| 文档 | 适合谁 | 内容 |
| --- | --- | --- |
| [技术架构](docs/TECHNICAL_ARCHITECTURE.md) | 开发者、研究者 | 设计、构建、索引、检索、问答和安全边界 |
...
| [发布前清单](docs/OPEN_SOURCE_CHECKLIST.md) | 项目维护者 | 代码、依赖、安全、数据迁移与验收 |
